    ...
    if strKey not in objCudacache:
        ...
        while True:
            ...
            strKernel = strKernel.replace(objMatch.group(),
                                          str(intSizes[intArg] if not torch.is_tensor(intSizes[intArg]) else intSizes[
                                              intArg].item()))
            ...
            for intArg in range(intArgs):
                strIndex.append(
                    '((' + strArgs[intArg + 1].replace('{', '(').replace('}', ')').strip() + ')*' + str(
                        intStrides[intArg] if not torch.is_tensor(intStrides[intArg]) else intStrides[
                            intArg].item()) + ')')
...
def cuda_launch(strKey: str):
    ...
    module = cupy.RawModule(code=objCudacache[strKey]['strKernel'], options=(
    '-I ' + os.environ['CUDA_HOME'], '-I ' + os.environ['CUDA_HOME'] + '/include'))
...
class costvol_func(torch.autograd.Function):
    ...
    def forward(self, tenOne, tenTwo):
        ...
        kernel_function = cuda_launch(cuda_kernel('costvol_out', '''
    extern "C" __global__ void __launch_bounds__(512) costvol_out(
        ...
        for (int intIndex = (blockIdx.x * blockDim.x) + threadIdx.x; intIndex < n; intIndex += blockDim.x * gridDim.x) {
            ...
            for (int intOy = intY - 4; intOy <= intY + 4; intOy += 1) {
                for (int intOx = intX - 4; intOx <= intX + 4; intOx += 1) {  // Corrected loop
                    {{type}} fltValue = 0.0f;
...
''', {
    ...
    'tenOut': tenOut
        ...
        kernel_function(
            ...
            block=tuple([512, 1, 1]),
            args=[cuda_int32(tenOut.shape[0] * tenOut.shape[2] * tenOut.shape[3]), tenOne.data_ptr(),
                  tenTwo.data_ptr(), tenOut.data_ptr()],
    ...
    def backward(self, tenOutgrad):
        ...
        tenOnegrad = tenOne.new_zeros(
            [tenOne.shape[0], tenOne.shape[1], tenOne.shape[2], tenOne.shape[3]]) if self.needs_input_grad[
                                                                                         0] == True else None
        tenTwograd = tenTwo.new_zeros(
            [tenTwo.shape[0], tenTwo.shape[1], tenTwo.shape[2], tenTwo.shape[3]]) if self.needs_input_grad[
                                                                                         1] == True else None

        if tenOnegrad is not None:
            kernel_function = cuda_launch(cuda_kernel('costvol_onegrad', '''
            ...
            kernel_function(
                ...
                block=tuple([512, 1, 1]),
                args=[cuda_int32(tenOnegrad.shape[0] * tenOnegrad.shape[2] * tenOnegrad.shape[3]), tenOne.data_ptr(),
                      tenTwo.data_ptr(), tenOutgrad.data_ptr(), tenOnegrad.data_ptr(), tenTwograd.data_ptr()],
                stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
            )

        if tenTwograd is not None:
            kernel_function = cuda_launch(cuda_kernel('costvol_twograd', '''
            ...
            kernel_function(
                ...
                block=tuple([512, 1, 1]),
                args=[cuda_int32(tenTwograd.shape[0] * tenTwograd.shape[2] * tenTwograd.shape[3]), tenOne.data_ptr(),
                      tenTwo.data_ptr(), tenOutgrad.data_ptr(), tenOnegrad.data_ptr(), tenTwograd.data_ptr()],
                stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
            )
